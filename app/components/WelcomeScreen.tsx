'use client';
import { MessageSquare, Brain, Zap, Search, Code2, Image, ArrowRight, Flame } from 'lucide-react';
...
export default function WelcomeScreen({ currentMode, onSuggestion, onModeChange }: {
  ...
  return (
    <div className="flex flex-col items-center justify-start md:justify-center h-full px-4 py-6 overflow-y-auto">
      ...
      <div className="mb-6 anim-up-1 flex items-center justify-center gap-1.5 md:flex-nowrap md:overflow-x-auto flex-wrap w-full max-w-[720px] pb-1 hide-scrollbar">
        {MODES.map(m => {
        ...
        {PROMPTS.map((p, i) => {
          ...
          return (
            <button key={i}
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = `${m.accent}30`; (e.currentTarget as HTMLElement).style.background = `${m.accent}08`; }}
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[10px] font-semibold uppercase tracking-[0.1em]" style={{ color: m.accent }}>{m.label}</span>
                </div>
                <p className="text-[13px] leading-snug" style={{ color: 'rgba(255,255,255,0.55)' }}>{p.text}</p>
              </div>
